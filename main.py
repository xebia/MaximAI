...
@app.post("/audio_text")
async def audio_text(file: UploadFile = File(...), user_id: str = "Emmy"):
    client = SpeechClient()
    model="latest_long"
    ...
    context = get_full_patient_context(user_id)
    # print(context)
    output = chatbot.invoke(
...
@app.post("/audio")
async def audio(file: UploadFile = File(...), user_id: str = "Emmy"):
    client = SpeechClient()
    model="latest_long"
    project_id="qwiklabs-gcp-02-c86bd22dbd03"
